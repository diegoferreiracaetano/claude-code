...
2. **Anti-pattern.** Name one anti-pattern `test_antipatterns.py` checks for. What would break in your run if the loop used it?
   → `test_no_integer_literal_iteration_cap_in_loop` statically walks `loop.py`'s AST and fails if it finds a `for _ in range(<int literal>)` or `while <var> < <int literal>` construct. If `loop.py` used, say, `for turn in range(2)` as its real stopping mechanism, `claim_04_neighbor_injury` and `claim_05_auto_collision` would break concretely: both traces show turn 1 = `lookup_policy`/`record_claim_fact` and turn 2 = `classify_claim`/`assess_severity`, with `route_to_adjuster` only arriving on **turn 3** (`evidence/system-1-claims-intake/run_sonnet_20260917_112347/traces/claim_04_neighbor_injury.jsonl` and `claim_05_auto_collision.jsonl`). A 2-turn cap would exit the loop right after turn 2 — before `route_to_adjuster` is ever called — turning two correctly-routed claims into silent non-decisions, with no error raised and no queue entry written.
...
11. **Push work down.** Defects the SQL query returned vs warm-tier total. Name the indexed query. Why does the model never see the full history?
    → `python -m shift_monitor run-shift --shift C --warm-db data/warm.sqlite --since 2026-04-16T00:00:00Z --recorded-response ...` returned **7** new defects (`evidence/system-4-shift-monitor/shift_run_output.txt`, `new=7`) out of **40** total defects seeded into the warm tier. The query is `WarmStore.defects_since(since_ts, limit)` (`shift_monitor/warm.py`, line 75), backed by `CREATE INDEX idx_defects_shift_ts ON defects(shift, ts)`. Reading the query body precisely: it's `WHERE ts > ? ORDER BY ts DESC LIMIT ?` — filtered by **timestamp only**; `gather_new_defects()` (`pipeline.py`) never passes `shift` into it at all, so `--shift C` labels the invocation but doesn't scope the SQL. Querying `warm.sqlite` directly confirms the 7 returned rows span shifts A, B, *and* C (2/2/3), not 13 shift-C-only rows — the index is composite on `(shift, ts)` (available for a shift-scoped query the codebase doesn't currently make) but the actual query only uses its `ts` ordering. The filtering still happens entirely inside SQL before any row reaches Python — `tests/test_us02_invocation_pipeline.py::test_gather_new_defects_has_no_python_side_filtering` asserts there is no post-query Python-side filter that could accidentally widen the slice back toward the full table. The model is only ever handed the 7 rows the query returned, never the other 33 — it just isn't a *per-shift* 33, it's a *stale-by-timestamp* 33.
